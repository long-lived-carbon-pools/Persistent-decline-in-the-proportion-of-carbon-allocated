...
model_dir=r"D:\all\data\npp"
shp_path=r"D:\all\data\世界底图\欧洲7类.shp"
out_dir=r"D:\all\2Extended Data Fig\Extended Data Fig. 7"
out_png=os.path.join(out_dir,"Extended Data Fig. 7.png")
...
label_map={2:"Arid",3:"Temperate",4:"Dfa",5:"Dfb"}
base_classes=[2,3,4,5]
agg_defs={102:("Warm",[1,2,3,4,5]),103:("Cold",[6,7]),104:("All",[2,3,4,5,6,7])}
plot_order=[2,3,4,5,102,103,104]
def rasterize_classes(shp_path,lon,lat):
    ...
    if not np.issubdtype(gdf[col].dtype,np.number):
        name2code={"B 干旱":2,"C 温带":3,"Dfa 冷带全湿炎夏":4,"Dfb 冷带全湿暖夏":5,"D 其他冷带":6,"D other":6,"E 寒带":7}
...
def legend_from_directive(ax,directive):
    if directive=="right":return {"loc":"lower right","bbox_to_anchor":None}
    if directive=="center":return {"loc":"lower center","bbox_to_anchor":None}
    if directive=="center_left":return {"loc":"lower center","bbox_to_anchor":(0.35,-0.02)}
    if directive=="center_right":return {"loc":"lower center","bbox_to_anchor":(0.65,-0.02)}
    return {"loc":"lower right","bbox_to_anchor":None}
def draw_block(ax,code,d,show_xlabel,show_ylabel,tag_text,legend_directive):
    t3=top3(d)
    if not t3:
        ax.set_axis_off()
        return
    ...
    if show_ylabel:ax.set_ylabel(tag_text,fontsize=22,labelpad=8)
    else:ax.set_ylabel("")
    label_txt=label_map[code] if code in base_classes else agg_defs[code][0]
    ax.text(0.02,0.92,label_txt,transform=ax.transAxes,ha="left",va="top",fontsize=15,fontweight="bold")
    cfg=legend_from_directive(ax,legend_directive)
    lg=ax.legend(fontsize=10,frameon=True,loc=cfg["loc"],bbox_to_anchor=cfg["bbox_to_anchor"],borderpad=0.05,handlelength=1.2,handletextpad=0.4,markerscale=0.6)
    if lg is not None:
...
def main():
    ...
    files=sorted(glob.glob(os.path.join(model_dir,"*.nc")))
    files=[f for f in files if not os.path.basename(f).startswith("2")]
    ...
    for fp in files:
        ...
        if common.size<WIN+1:
            dsB.close()
            continue
        A=daA.sel(year=common)
        B=daB.sel(year=common)
        if (B.sizes.get('latitude')!=A.sizes['latitude']) or (B.sizes.get('longitude')!=A.sizes['longitude']) or (not np.allclose(B['latitude'],A['latitude'])) or (not np.allclose(B['longitude'],A['longitude'])):
            B=B.interp(latitude=A['latitude'],longitude=A['longitude'])
        A=A.load().values.astype(np.float32)
        B=B.load().values.astype(np.float32)
        dsB.close()
        Ad=np.diff(A,axis=0)
        Bd=np.diff(B,axis=0)
        yrs_diff=common[1:]
    ...
    directives=["right","center","right","right","right","center_left","right"]
    for i in range(7):
        r=i//4
        c=i%4
        ax=axes[r,c]
        ...
        draw_block(ax,code,d,show_xlabel=False,show_ylabel=(c==0),tag_text="r_diff",legend_directive=directives[i])
    fig.delaxes(axes[1,3])
    directives2=["right","right","right","right","right","center_right","center_right"]
    for i in range(7):
        r=2+(i//4)
        c=i%4
        ax=axes[r,c]
        ...
        draw_block(ax,code,d,show_xlabel=(r==3),show_ylabel=(c==0),tag_text="r_raw",legend_directive=directives2[i])
